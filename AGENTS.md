...
<!-- PROJECT-CUSTOM-START: task-intake-routing -->
## Roteamento obrigatório antes da implementação

Antes de modificar código, classifique a solicitação seguindo `.aiox-core/development/tasks/route-task-intake.md`.
Quando a descrição e os arquivos alvo forem conhecidos, execute:

```
printf '%s' '<task-json>' | node .aiox-core/scripts/route-task-intake.js
```

1. `quick`: ajuste localizado e de baixo risco. Exige revisão de escopo e risco, revisão completa do diff e validação direcionada.
2. `standard`: correção funcional ou trabalho delimitado. Usa o Story Development Cycle e o QA Gate padrão.
3. `full`: arquitetura, segurança, autenticação, autorização, banco, migration, contrato de API, infraestrutura, produção, cobrança, pagamento, operação destrutiva ou breaking change. Exige Spec Pipeline antes da implementação e todos os Gates.

Riscos da rota `full` sempre prevalecem sobre uma solicitação explícita de rota `quick`. Quando o roteamento falhar ou os arquivos alvo forem desconhecidos, use `standard`.
<!-- PROJECT-CUSTOM-END: task-intake-routing -->
