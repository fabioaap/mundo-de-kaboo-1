-- T2.2: Popula brand_id no INSERT de audit_log dentro de redeem_voucher
-- Supersedes: 20260620100000_t01_redeem_voucher_brand_guard.sql
-- Mudança: apenas o INSERT em audit_log ganha a coluna brand_id,
-- passando profile_record.brand_id (já populado/validado pelo brand guard do T0.1).
-- Todo o restante da função é idêntico ao T0.1.
...
BEGIN
  ...
  IF voucher_record.batch_id IS NOT NULL THEN
    ...
    IF FOUND AND v_batch_record.model_snapshot IS NOT NULL THEN
      ...
      INSERT INTO public.audit_log (actor_id, action, entity_type, entity_id, details, brand_id)
      VALUES (
