...
-- Previously redeem_voucher granted only the snapshot's collection_ids (the kit),
-- so the user owned the kit but every "Ler/Ouvir" hit the upsell modal.
--
-- This migration:
--   1) updates redeem_voucher to also grant each kit's kit_book_ids on redeem;
--   2) backfills existing grants (already-redeemed users get their kit books now).
...
-- Backfill: existing grants for kits get their linked books too (idempotent).
INSERT INTO public.user_content_grants (user_id, collection_id, voucher_id, granted_at, expires_at)
SELECT g.user_id, book_id::UUID, g.voucher_id, g.granted_at, g.expires_at
FROM public.user_content_grants g
JOIN public.collections c ON c.id = g.collection_id AND c.collection_type = 'kit'
CROSS JOIN LATERAL unnest(c.kit_book_ids) AS book_id
WHERE c.kit_book_ids IS NOT NULL
ON CONFLICT (user_id, collection_id, voucher_id) DO NOTHING;
