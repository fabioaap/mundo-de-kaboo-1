...
- [x] ~~**Estado "material fora do voucher" (degustação → upsell)**~~ — feito 2026-06-15: modal de upsell (`components/VoucherUpsellModal.tsx`) + gate central em `App.navigate` (e no render do modal de detalhes p/ deep-link) usando `canAccessCollection(contentGrants, id)`. Material não coberto abre o modal "Comprar na loja" em vez do conteúdo. CTA aponta para `VOUCHER_UPSELL_STORE_URL` (**placeholder — confirmar URL da loja**). **Pendente:** verificação ao vivo com conta *viewer* + grants (admin não passa pelo gate); gate de players via deep-link (cobertos via navigate in-app).
