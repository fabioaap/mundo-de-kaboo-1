...
- [~] **Catálogo real da Central Coruja** (≥3 coleções, 2 personagens, 1 ativo por tipo, visíveis) — **checklist/estrutura + auditoria** em `docs/CHECKLIST-CATALOGO-CENTRAL-CORUJA.md` (2026-06-15). Estado real: ✅ 18 coleções publicadas (livro 18, áudio 7, vídeo **1**, material 10); 🔴 **0 personagens** da marca; ✅ os `character_ids` da **Kaboo** em 12 coleções foram **limpos** (migration `20260615200000`); 🔴 **0 viewer da Coruja** p/ validar (admin `admin@mundodekaboo.dev` já gerencia as duas marcas). **Falta:** cadastrar ≥2 personagens Coruja + revincular às coleções, criar viewer da Coruja, reforçar vídeo (só 1), homologar visibilidade logado como Coruja. (Conteúdo)
- [~] **Hardening operacional** — **RUNBOOK criado** (2026-06-15): `docs/RUNBOOK-OPERACIONAL.md` cobre deploy (GH Pages CI + smoke), rollback, migrations (sem staging), env/segredos, monitoramento (CI smoke + `get_advisors` + `get_logs`), feature flags/kill switches por marca, offline (SW `kaboo-offline-v1`) e playbooks de incidente. Auditoria executada (advisors security/perf). **Hardening de DB aplicado (2026-06-15):** `search_path=public` nas 9 funções flagueadas + `EXECUTE` revogado de `anon`/`PUBLIC` nas triggers e nas RPCs admin (migrations `20260615190000`/`20260615191000`); verificado sem quebra. **Pendências restantes (não-bloqueantes p/ go-live só-Kaboo):** sem staging; sem error tracking (Sentry); auth no dashboard (OTP expiry/leaked-password/MFA); bucket público `collections` lista arquivos; advisors perf (RLS initplan, policies permissivas múltiplas, FK sem índice); Cloudflare Access (G1). (Infra)
- [x] **URLs white-label agora editáveis por marca no White Label** (2026-06-15). Loja de upsell, captação de lead e contato de suporte viraram `brand_links` em `menu_config` (sem migration — `get_brand_bootstrap` já retorna `menu_config`), expostos em `BrandSettings` (`store_url`/`lead_capture_url`/`support_contact_url`) e editáveis na seção "Links da marca" do `AdminWhiteLabelScreen`. Consumidores: upsell (`App`) e login (lead/suporte) leem das settings com fallback para o resolver por slug/constantes. Valores corretos por marca já válidos sem edição: Kaboo→Empatia Editora, Central Coruja→Universo Educacross. Tema/cores/logo já eram white-label; componentes da sessão usam tokens `brand-*`. Typecheck 0, unit 94/95 (falha pré-existente não-relacionada).
...
- [ ] **[EPIC] Migração da infra white-label para Educacross (Docker)** — instalar as 3 instâncias em containers Docker na infra da Educacross: **(1)** app Kaboo (mondo-de-kaboo), **(2)** app Central Coruja (white-label Coruja), **(3)** wiki Docusaurus (`docs/`). Cada instância é um serviço isolado (imagem, variáveis de ambiente, domínio/subdomínio próprio). Pré-requisitos: definir estratégia de build (multi-stage Dockerfile por app), orquestração (Docker Compose ou k8s), pipeline CI/CD para push de imagem + rollout, e variáveis de ambiente por marca (`VITE_BRAND_SLUG`, Supabase keys). Substitui o deploy atual via GitHub Pages. (Infra)
