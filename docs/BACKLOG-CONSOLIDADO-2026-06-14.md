...
- [~] **Hardening operacional** — **RUNBOOK criado** (2026-06-15): `docs/RUNBOOK-OPERACIONAL.md` cobre deploy (GH Pages CI + smoke), rollback, migrations (sem staging), env/segredos, monitoramento (CI smoke + `get_advisors` + `get_logs`), feature flags/kill switches por marca, offline (SW `kaboo-offline-v1`) e playbooks de incidente. Auditoria executada (advisors security/perf). **Pendências de hardening (não-bloqueantes p/ go-live só-Kaboo):** sem staging; sem error tracking (Sentry); advisors security WARN (9 funções DEFINER sem `search_path`; DEFINER executáveis por `anon` incl. triggers; bucket público lista; auth OTP/leaked-password/MFA); advisors perf (RLS initplan, policies permissivas múltiplas, FK sem índice); Cloudflare Access (G1). (Infra)
