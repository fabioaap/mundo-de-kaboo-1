...
- [ ] **URLs/copy white-label hardcoded → mover para `brand_settings`** (White-label). Auditoria 2026-06-15: o tema (cores/logo/hero) já é white-label e os componentes desta sessão usam tokens `brand-*` (o modal de upsell adota o `brand-primary` da marca; card bloqueado é neutro). **Mas há URLs presas à Kaboo:** loja de upsell (`VOUCHER_UPSELL_STORE_URL` = educabox — agora resolvida por slug em `getVoucherUpsellStoreUrl`, falta a URL da Central Coruja), `LEAD_CAPTURE_URL` (empatiaeditora), `SUPPORT_CONTACT_URL` (suporte@mundodekaboo.com). Ideal: virar campos editáveis no White Label (`brand_settings`). **Pendente:** URL da loja da Central Coruja.
