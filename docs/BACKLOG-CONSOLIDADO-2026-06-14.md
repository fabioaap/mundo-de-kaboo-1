...
- [~] **Catálogo real da Central Coruja** (≥3 coleções, 2 personagens, 1 ativo por tipo, visíveis) — **checklist/estrutura + auditoria** em `docs/CHECKLIST-CATALOGO-CENTRAL-CORUJA.md` (2026-06-15). Estado real: ✅ 18 coleções publicadas (livro 18, áudio 7, vídeo **1**, material 10); 🔴 **0 personagens** da marca (e 12 coleções referenciam `character_ids` da **Kaboo** — cross-brand a corrigir); 🔴 **0 usuários/admin da Coruja** (sem membership ninguém cadastra/valida pela UI). **Falta:** cadastrar ≥2 personagens Coruja, remapear/limpar character_ids, criar viewer+admin da Coruja, reforçar vídeo, homologar visibilidade logado como Coruja. (Conteúdo)
